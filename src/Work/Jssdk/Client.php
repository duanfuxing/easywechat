...
class Client extends BaseClient
{
    ...
    protected function agentConfigSignature($agentId, string $url = null, string $nonce = null, $timestamp = null): array
    {
        ...
        return [
            ...
            'timestamp' => $timestamp,
            'url' => $url,
            'signature' => $this->getTicketSignature($this->getAgentTicket(false, 'agent_config', $agentId)['ticket'], $nonce, $timestamp, $url),
        ];
    }
     ...
     * @throws \Psr\SimpleCache\InvalidArgumentException
     */
    public function getAgentTicket(bool $refresh = false, string $type = 'agent_config', $agentId = '')
    {
        $agentId = empty($agentId) ? $this->getAgentId() : $agentId;

        $cacheKey = sprintf('easywechat.work.jssdk.ticket.%s.%s.%s', $type, $this->getAppId(), $agentId);
